...
float4 main(PostProcessingInput input) : SV_Target
{
    ...
    {
        samples += SceneTexture.Sample(PointSample, input.sceneUV + float2(0.0f, height * i)) * weights[i].x +
                   SceneTexture.Sample(PointSample, input.sceneUV - float2(0.0f, height * i)) * weights[i].x;
    }
